...
c If this is a new integration, integrate all the objects to a common epoch.
  ...
  20    open (23,file=outfile(3),status='old',access='append',err=20)
     ...
     %    rceh,stat,id,epoch,ngf,opt,ngflag,params)
...
c Main integration
     ...
     %  lmem,mdt_mvs,mco_h2mvs,mco_mvs2h,params)
c
      if (algor.eq.9) call mal_hcon (time,tstart,tstop,dtout,algor,h0,
     %  tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,
     %  rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem,
     %  lmem,mdt_mvs,mco_iden,mco_iden,params)
c
...
c
c Tweak #3 is also implemented here: pass each particle's
c optional p1,p2,p3 parameters here in the array params(3,nbod).
c These parameters can then be used to drive additional orbit evolution.
c
...
c
c     migration accelerations
      call migrate(time,nbod,m,x,v,a,params)
c      
c------------------------------------------------------------------------------
c
      return
...
c Author: Joe Hahn
c
c Apply accelerations that drive the planet-migration that is decribed in
c Hahn & Malhotra, AJ, 130, 2392. Each planet's migration path is described
c by two parameters: a migration timescale p1=tau (in years), and a radial 
c distance p2=Delta (in AU). The planet's semimajor axes then evolves
c as a(t)=a(0)-Delta*exp(-t/tau) in the absence of other perturbations.
c
c------------------------------------------------------------------------------
c
      subroutine migrate (time,nbod,m,x,v,a,params)
c
      implicit none
      include 'mercury.inc'
c
...
c Local
      integer j,k
      real*8 tau,tau_days,da,t_factor,r,v2,one_over_a,factor
c
c------------------------------------------------------------------------------
c
      do j = 2, nbod

c	migration parameters, in years, days, and AU.
	tau=params(1,j)
        tau_days=tau*365.25d0
	da=params(2,j)

c	Increment the planet's accelerations.
	if (tau .gt. 0.d0) then
	  t_factor=0.5d0*exp(-time/tau_days)/tau_days
	  r=x(1,j)**2+x(2,j)**2+x(3,j)**2
          r=sqrt(r)
	  v2=v(1,j)**2+v(2,j)**2+v(3,j)**2
	  one_over_a=2.d0/r-v2/K2
	  factor=t_factor*da*one_over_a
...
c Check for collisions
     ...
     %  jhit,thit,dhit,algor,ngf,ngflag,params)
c
...
c Check for collisions with the central body
     ...
     %  nhit,jhit,thit,dhit,algor,ngf,ngflag,params)
c
...
c
     ...
     %      ngf,ngflag,opt,nce,ice,jce,params)
          do k = 2, nbod
...
c If objects had close encounters, advance H_K using Bulirsch-Stoer instead
     ...
     %    jxvclo,outfile,mem,lmem,mfo_hkce,params)
      end if
c
c Advance solar Hamiltonian for H/2
      mvsum(1) = 0.d0
...
c
     ...
     %  ngflag,colflag,ce,nce,ice,jce,nclo,iclo,jclo,dclo,tclo,ixvclo,
     %  jxvclo,outfile,mem,lmem,force,params)
c
      implicit none
      include 'mercury.inc'
c
...
c Include user-defined accelerations if required
      if (opt(8).eq.1) then
        call mfo_user (time,jcen,nbod,nbig,m,x,v,acor,params)
        do j = 2, nbod
          a(1,j) = a(1,j) + acor(1,j)
          a(2,j) = a(2,j) + acor(2,j)
          a(3,j) = a(3,j) + acor(3,j)
...
c
c------------------------------------------------------------------------------
c correction by de Souza Torres & Anderson (2008)
      do j=2, nbod
        STAT(j)=0
      end do
c------------------------------------------------------------------------------
...
c Eliminate unwanted objects
      do k = 1, nelim
        do j = elim(k) - k + 1, elim(k+1) - k - 1
          ...
          ngf(3,j) = ngf(3,l)
          ngf(4,j) = ngf(4,l)
          params(1,j) = params(1,l)
          params(2,j) = params(2,l)
          params(3,j) = params(3,l)          
        end do
      end do
c
...
c
     ...
     %  rho,rceh,stat,id,epoch,ngf,opt,ngflag,params)
c
      implicit none
      include 'mercury.inc'
c
